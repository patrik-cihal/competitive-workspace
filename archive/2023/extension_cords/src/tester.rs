...
<<<<<<<< HEAD:extension_cords/src/tester.rs
    let mut paths = std::fs::read_dir("./extension_cords/tests/")
========
    let mut paths = std::fs::read_dir("./b_going_to_the_cinema/tests/")
>>>>>>>> 53fdd8eaa5da12868d0d145b94f93ce27a9e688a:b_going_to_the_cinema/src/tester.rs
        .unwrap()
        .map(|res| res.unwrap())
        .collect::<Vec<_>>();
