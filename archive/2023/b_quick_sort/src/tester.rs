...
<<<<<<<< HEAD:b_fedya_and_array/src/tester.rs
    let mut paths = std::fs::read_dir("./b_fedya_and_array/tests/")
========
    let mut paths = std::fs::read_dir("./b_quick_sort/tests/")
>>>>>>>> 53fdd8eaa5da12868d0d145b94f93ce27a9e688a:b_quick_sort/src/tester.rs
        .unwrap()
        .map(|res| res.unwrap())
        .collect::<Vec<_>>();
