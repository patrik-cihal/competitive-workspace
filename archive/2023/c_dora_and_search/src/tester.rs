...
<<<<<<<< HEAD:c_dora_and_search/src/tester.rs
    let mut paths = std::fs::read_dir("./c_dora_and_search/tests/")
========
    let mut paths = std::fs::read_dir("./a_greatest_convex/tests/")
>>>>>>>> 53fdd8eaa5da12868d0d145b94f93ce27a9e688a:a_greatest_convex/src/tester.rs
        .unwrap()
        .map(|res| res.unwrap())
        .collect::<Vec<_>>();
