...
<<<<<<<< HEAD:d_triangle_coloring/src/tester.rs
    let mut paths = std::fs::read_dir("./d_triangle_coloring/tests/")
========
    let mut paths = std::fs::read_dir("./a_make_it_beautiful/tests/")
>>>>>>>> 53fdd8eaa5da12868d0d145b94f93ce27a9e688a:a_make_it_beautiful/src/tester.rs
        .unwrap()
        .map(|res| res.unwrap())
        .collect::<Vec<_>>();
