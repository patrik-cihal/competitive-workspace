...
pub(crate) fn run_tests() -> bool {
    ...
    let yellow = "\x1B[33m";
    let def = "\x1B[0m";
<<<<<<<< HEAD:d_moscow_gorillas/src/tester.rs
    let time_limit = std::time::Duration::from_millis(2000);
    let mut paths = std::fs::read_dir("./d_moscow_gorillas/tests/")
========
    let time_limit = std::time::Duration::from_millis(4000);
    let mut paths = std::fs::read_dir("./d_many_perfect_squares/tests/")
>>>>>>>> 53fdd8eaa5da12868d0d145b94f93ce27a9e688a:d_many_perfect_squares/src/tester.rs
        .unwrap()
        .map(|res| res.unwrap())
        .collect::<Vec<_>>();
